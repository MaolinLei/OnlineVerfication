...
{  
    ...
    expection_4Obj_4Next_Read(0) = msg.poses[n].pose.position.x + 0.0;
    expection_4Obj_4Next_Read(1) = msg.poses[n].pose.position.y;
    expection_4Obj_4Next_Read(2) = msg.poses[n].pose.position.z;

    expection_4Obj_4Next_Read(3) = msg.poses[msg.poses.size()-1].pose.position.x + 0.0 ;
...
int main(int argc, char **argv)
{
    ...
    YAML::Node arm_param_config = YAML::LoadFile("/home/oem/ModularBot/config/ModularBot.yaml");
    double dt;
    dt = 0.01;
    ros::Rate rate(1./dt);
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    //R_Orientation = fcl::Matrix3d::Identity();
      ...
      if(j <=1)
      {   
      B_4distance_Constraint_horizon(j,0) = (0.025000*(1)-0.015000000*(j)) - B_4distance_Constraint(0,0);
...
cout << "Vector_Constraint_Aconstraint_upper_matrix= "<< endl<<Vector_Constraint_Aconstraint_upper_matrix << endl; */
	...
	QProblem example( horizon*Number_Joint, NumberConstraint + NumberConstraint );
    ...
    Eigen::VectorXd First_Hieranchy_result(Number_Joint);
        for(int i =0; i<Number_Joint; i++)
        First_Hieranchy_result(i) = xOpt[i] ;
    ...
    Eigen:: MatrixXd Eigen_4Constraint_2ndHer_A (Number_Joint* horizon + 3 * Numer_H, Number_Joint * horizon);  
    
    Eigen:: MatrixXd Equally_Constraint(3 * Numer_H, Number_Joint*horizon);
    Equally_Constraint.setZero();
    
    for(int i=0;i<Numer_H;i++)
    {
       Equally_Constraint.leftCols(nj) = Jacobian_4Robot_End_4velocity; 

    }
 
    Eigen::MatrixXd H_Her;
    Eigen::MatrixXd g_Her;
    ...
    Eigen_4Constraint_2ndHer_A.topRows(NumberConstraint) = Matrix_Constraint_4pos_;
    Eigen_4Constraint_2ndHer_A.middleRows(NumberConstraint,3 * Numer_H) =  Equally_Constraint;

    //Eigen_4Constraint_2ndHer_A.bottomRows(NumberConstraint) = Matrix_Constraint_4acc_ ;

///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    //Eigen:: VectorXd Vector_Constraint_Aconstraint_lower_Hier(Number_Joint * horizon + Number_Joint * horizon + 3 * Numer_H);  
    //Eigen:: VectorXd Vector_Constraint_Aconstraint_upper_Hier(Number_Joint * horizon + Number_Joint * horizon + 3 * Numer_H);  


    Eigen:: VectorXd Vector_Constraint_Aconstraint_lower_Hier(Number_Joint * horizon + 3 * Numer_H);  
    Eigen:: VectorXd Vector_Constraint_Aconstraint_upper_Hier(Number_Joint * horizon + 3 * Numer_H);  

    Vector_Constraint_Aconstraint_lower_Hier.topRows(NumberConstraint) = Vector_Constraint_4pos_lower; 
    //Vector_Constraint_Aconstraint_lower.middleRows(NumberConstraint, NumberConstraint) = Vector_Constraint_4acc_lower;
    //Vector_Constraint_Aconstraint_lower.bottomRows(horizon) =    B_4distance_Constraint_horizon;
    Vector_Constraint_Aconstraint_lower_Hier.middleRows(NumberConstraint, 3 * Numer_H) = Jacobian_4Robot_End_4velocity * First_Hieranchy_result ;
    
    // Vector_Constraint_Aconstraint_lower_Hier.bottomRows(NumberConstraint) = Vector_Constraint_4acc_lower;

    Vector_Constraint_Aconstraint_upper_Hier.topRows(NumberConstraint) = Vector_Constraint_4pos_upper;
    Vector_Constraint_Aconstraint_upper_Hier.middleRows(NumberConstraint, 3 * Numer_H) = Jacobian_4Robot_End_4velocity * First_Hieranchy_result;
    
    // Vector_Constraint_Aconstraint_upper_Hier.bottomRows(NumberConstraint) = Vector_Constraint_4acc_upper;

////////////////////////////////////////
	// USING_NAMESPACE_QPOASES

    qpOASES::real_t* H_qpoases_Her;
    H_qpoases_Her = (qpOASES::real_t*)malloc(H_Her.rows()* H_Her.cols()*sizeof(qpOASES::real_t)); //rows lie
	MPC_OPTIMAL_.matrix_to_real(H_qpoases_Her, H_Her, H_Her.rows(), H_Her.cols());


	qpOASES::real_t* g_qpoases_Her;
    Eigen::MatrixXd g_matrix_Her = g_Her;
    g_qpoases_Her = (qpOASES::real_t*)malloc(g_matrix_Her.rows()*sizeof(qpOASES::real_t));
  	MPC_OPTIMAL_.matrix_to_real(g_qpoases_Her, g_matrix_Her, g_matrix_Her.rows(), 1);

    qpOASES::real_t* A_qpoases_Her;
    A_qpoases_Her = (qpOASES::real_t*)malloc(Eigen_4Constraint_2ndHer_A.rows()*Eigen_4Constraint_2ndHer_A.cols()*sizeof(qpOASES::real_t)); //rows lie
	MPC_OPTIMAL_.matrix_to_real(A_qpoases_Her, Eigen_4Constraint_2ndHer_A, Eigen_4Constraint_2ndHer_A.rows(), Eigen_4Constraint_2ndHer_A.cols());

    qpOASES::real_t* lbA_qpoases_Hier;
    Eigen::MatrixXd Vector_Constraint_Aconstraint_lower_matrix_Hier = Vector_Constraint_Aconstraint_lower_Hier;
    lbA_qpoases_Hier = (qpOASES::real_t*)malloc(Vector_Constraint_Aconstraint_lower_matrix_Hier.rows()*sizeof(qpOASES::real_t)); //rows lie
	MPC_OPTIMAL_.matrix_to_real(lbA_qpoases_Hier, Vector_Constraint_Aconstraint_lower_matrix_Hier, Vector_Constraint_Aconstraint_lower_matrix_Hier.rows() , 1);

    qpOASES::real_t* ubA_qpoases_Hier;
    Eigen::MatrixXd Vector_Constraint_Aconstraint_upper_matrix_Hier = Vector_Constraint_Aconstraint_upper_Hier;
    ubA_qpoases_Hier = (qpOASES::real_t*)malloc(Vector_Constraint_Aconstraint_upper_matrix_Hier.rows()*sizeof(qpOASES::real_t)); //rows lie
	MPC_OPTIMAL_.matrix_to_real(ubA_qpoases_Hier, Vector_Constraint_Aconstraint_upper_matrix_Hier, Vector_Constraint_Aconstraint_upper_matrix_Hier.rows() , 1);


	QProblem example_Her( horizon*Number_Joint, Number_Joint* horizon + 3 * Numer_H  );

	Options options_Her;
	example_Her.setOptions( options_Her );

	example_Her.init( H_qpoases_Her,g_qpoases_Her,A_qpoases_Her,lb_qpoases,ub_qpoases,lbA_qpoases_Hier,ubA_qpoases_Hier, nWSR, 0);

	real_t xOpt_Her[Number_Joint*horizon];
	real_t yOpt_Her[Number_Joint* horizon + Number_Joint* horizon  + 3 * Numer_H];
	example_Her.getPrimalSolution( xOpt_Her ); 
	example_Her.getDualSolution( yOpt_Her );

	//printf( "\nxOpt = [ %e, %e , %e, %e, %e, %e];    objVal = %e\n\n", 
	//		xOpt[0],xOpt[1],xOpt[2],xOpt[3],xOpt[4],xOpt[5],example.getObjVal() );


    for(int i=0;i<Number_Joint* horizon + 3 * Numer_H;i++)
    {   
         // std::cout << "Y[]" << yOpt_Her[i] << std::endl;
    }


    for(int i=0;i<Number_Joint;i++)
    {
    acceleration(i) = xOpt_Her[i]/dt - velocity_inLastTIme(i)/dt;
    velocity_inLastTIme(i) = xOpt_Her[i];
    }
